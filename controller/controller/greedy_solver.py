...
class GreedyOfflineAlgorithm(ServingSolver):
    ...
    def __init__(self, request_sorter: RequestSorter, online_algo: OnlineAlgorithm) -> None:
        super().__init__()
...
class PlacementAlgorithm(OnlineAlgorithm):
    ...
    def __init__(self, acc_ascending: bool = True, fps_ascending: bool = True) -> None:
        ...
        Args:
            acc_ascending (bool, optional): When true, models are considered from lower accuracy
            to higher. When false, models are considered from higher accuracy to lower. Defaults to True.
            fps_ascending (bool, optional): When true, servers are considered from lower additional fps
            to higher. When false, models are considered from higher additional fps to lower. Defaults to True.
        """
    ...
    def best_config(self, request: SessionRequest, serving_system: ServingSystem) -> SessionConfiguration:
        ...
        for i in mod_range:
            ...
            tx_latency = estimate_transmission_latency(models[i].input_size, request.transmission_speed)
            max_serving = request.max_latency - tx_latency

            # Traverse servers
            servers = serving_system.servers_by_model[models[i].id]
            serv_range = self._get_range(servers, request.arrival_rate, self.fps_ascending)
            ...
            for j in serv_range:
                ...
                if (
                    serving_system.total_serving_latency(arrival_rate_dict, profiling_data)
                    <= max_serving
                ):
