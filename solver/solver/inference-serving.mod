...
#         + (3/2) * (beta[j,k] / (1 - (sum{l in R}(I[l,j,k]*lambda[l])) * alpha[j,k]))
#         + (alpha[j,k]/2) * ((sum{l in R}(I[l,j,k]*lambda[l]))*alpha[j,k] + 2) / (1 - ((sum{l in R}(I[l,j,k]*lambda[l]))^2)*(alpha[j,k]^2))));
# maximize minimum M
maximize min_reward:
    MinimumReward;

## Constraints
subject to reward{i in R}:
    Reward[i] = sum{(j,k) in SERVED}(I[i,j,k]*accuracy[j] / (input_size[j]/tau[i]
        + (3/2) * (beta[j,k] / (1 - (sum{l in R}(I[l,j,k]*lambda[l])) * alpha[j,k]))
        + (alpha[j,k]/2) * ((sum{l in R}(I[l,j,k]*lambda[l]))*alpha[j,k] + 2) / (1 - ((sum{l in R}(I[l,j,k]*lambda[l]))^2)*(alpha[j,k]^2))));
subject to minimum_reward{i in R}:
    MinimumReward <= Reward[i];
...
subject to MaxThroughput{(j,k) in SERVED}:
    sum{i in R}(I[i,j,k]*lambda[i]) <= max_throughput[j,k];